...
public class ServerSyncServiceTests
{
    ...
    [Fact]
    public async Task SyncAsync_WhenGameTypeIsNonCoD4x_SkipsCoD4xRconAndStillRunsQuerySync()
    {
        _mockParser.SetupGet(p => p.ConnectedPlayers)
            .Returns(new Dictionary<int, PlayerInfo>());
    ...
    [Fact]
    public async Task SyncAsync_WhenGameTypeIsNonCoD4x_DoesNotInvokeCoD4xReconciliation()
    {
        _mockParser.SetupGet(p => p.ConnectedPlayers)
            .Returns(new Dictionary<int, PlayerInfo>());
